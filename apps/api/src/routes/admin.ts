...
const productInput = z.object({ title: z.string().trim().min(3).max(140), slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), description: z.string().trim().min(20).max(5000), pricePaise: z.number().int().min(100).max(10_000_000), coverUrl: z.string().url().optional(), fileKey: z.string().max(500).optional(), published: z.boolean().default(false) });
async function requireAdmin(app: FastifyInstance, request: any, reply: any) { await app.authenticate(request, reply); if (reply.sent) return; if (request.user.role !== 'ADMIN') return reply.code(403).send({ message: 'Admin access required.' }); }
