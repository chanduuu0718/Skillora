      ...
      const checkout = new window.Razorpay({
        ...
        handler: async (response: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) => {
          try {
            ...
            setMessage('Payment successful. Your resource is now in My Library.');
    ...
    <main className="app-shell">
      <nav className="topbar">
        ...
        <div className={`nav-links ${mobileOpen ? 'mobile-open' : ''}`}>
          <a href="#products" onClick={() => setMobileOpen(false)}>Explore</a>
          <a href="#how-it-works" onClick={() => setMobileOpen(false)}>How it works</a>
          {user && <a href="#library" onClick={() => setMobileOpen(false)}>My Library</a>}
        </div>
        <div className="nav-actions">
          {user ? <><span className="user-pill">{user.name.split(' ')[0]}</span><button className="ghost-button" onClick={handleLogout}><LogOut size={16} /> Sign out</button></> : <><button className="ghost-button" onClick={() => setAuthMode('login')}>Log in</button><button className="primary-button" onClick={() => setAuthMode('register')}>Create account</button></>}
        </div>
      </nav>
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-copy">
          ...
          <h1>Useful knowledge, <span>ready when you are.</span></h1>
          <p>Skillora turns practical guides, templates and focused learning resources into simple digital products you can access instantly and keep in your library.</p>
        ...
        <div className="hero-card"><div className="hero-card-top"><span>SKILLORA LIBRARY</span><span>LIVE</span></div><div className="product-orbit"><div className="orbit-core">SK</div></div><h2>Build your own advantage.</h2><p>Focused resources designed to help you prepare, create and move forward without information overload.</p><div className="price-row"><strong>{products.length || '—'}</strong><span>resources available</span></div><a className="card-button" href="#products">Browse library <ArrowRight size={17} /></a></div>
      </section>

      <section className="section" id="products">
        <div className="section-heading"><div><div className="section-kicker">THE SKILLORA LIBRARY</div><h2>Small resources. Real progress.</h2></div><span className="catalog-count">{products.length} resources</span></div>
        {message && <div className="notice">{message}</div>}
        <div className="product-grid">
          {products.length ? products.map((product, index) => <article className="product-card" key={product.id}>
            <div className={`product-art art-${(index % 3) + 1}`}><span>{String(index + 1).padStart(2, '0')}</span><Sparkles size={24} /></div>
            <div className="product-meta"><span>Digital resource</span><span>₹{(product.pricePaise / 100).toLocaleString('en-IN')}</span></div>
            <h3>{product.title}</h3><p>{product.description}</p>
            {owned.has(product.id) ? <a className="card-link owned-link" href="#library"><Check size={17} /> In your library</a> : <button className="card-link" disabled={busy} onClick={() => buy(product)}>Get this resource <ArrowRight size={17} /></button>}
          </article>) : <div className="empty-state"><Sparkles size={24} /><h3>Preparing the library</h3><p>Add your first product from the admin dashboard and it will appear here.</p></div>}
        </div>
      </section>
